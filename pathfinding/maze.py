...
class Display:
    def __init__(self, maze, padding):
        ...
        self.drawing = svg.Drawing(self.filename, size=(100*pct, 100*pct))
        self.drawing.add_stylesheet("styles.css", title="stylesheet")

        bg = self.drawing.rect((0*pct, 0*pct), (100*pct, 100*pct), fill="navy")
                ...
                for direction, start, end in line_data:
                    if not cell[direction]:
                        for i, walls in enumerate(walls_removed):
                            if len(walls) == 1 and (column, row) in walls:
                                death = 5 / len(walls_removed) * i + 1
                            if len(walls) > 1 and (column, row, direction) in walls:
                                lifetime = 5 / len(walls_removed) * i + 1
                        self.draw_square(top_left, lifetime, death)
                    else:
                        lifetime = None
...
class Maze:
    ...
    def generate(self):
        self.grid[0][0]["left"] = False
        self.grid[-1][-1]["right"] = False
        while self.stack:
            walls = self.step()
            yield walls
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Run code again for an enhanced experience. An error from your computer's side was detected. We will attempt to adjust this automatically; if this error persists, try rebooting your device.")
